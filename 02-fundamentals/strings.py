...
def jedna(datum):
    datum = datum.split(". ")
    pom="-"
    ceske_datum=datum[-1]+pom+datum[-2]+pom+datum[-3]
    print(ceske_datum)
def dva(souslovi):
    pomlcka="_"
    volba =input("Volba: a) identifikatoru pro promenne pouzivane v Pythonu \n"
          "       b) identifikátor pro camel syntax v JS\n")
    if volba == 'a':
        souslovi=souslovi.lower().replace(" ","_")
        print(souslovi)
    elif volba == 'b':
        souslovi=souslovi.split(" ")
        pocet = len(souslovi)
        vysledek=souslovi[0].lower()
        for i in range(pocet):
            if i!=0:
             vysledek+=souslovi[i].capitalize()
        print(vysledek)

import random
import string
def tri(pocet_uzivatelu):
    passwords= []
    specialni_charaktery= "-/+*"
    for i in range(pocet_uzivatelu):
        password = ''.join(random.choices(string.ascii_uppercase, k=3))+''.join(random.choices(string.ascii_lowercase, k=3))+random.choice(specialni_charaktery)+''.join(str(random.randint(1, 9)) for _ in range(3))
        passwords.append(password)

    return passwords
jedna("12. 10. 2020")
dva("To je proměnná v Pythonu")
passwords=tri(3)
